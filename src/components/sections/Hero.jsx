  ...
  return (
    <section id="home" className="min-h-screen flex items-center relative overflow-hidden hero-bg bg-[radial-gradient(ellipse_at_20%_50%,rgba(201,168,76,0.08)_0%,transparent_60%),radial-gradient(ellipse_at_80%_20%,rgba(201,168,76,0.05)_0%,transparent_50%)] dark:bg-[radial-gradient(ellipse_at_20%_50%,rgba(201,168,76,0.06)_0%,transparent_60%),radial-gradient(ellipse_at_80%_20%,rgba(201,168,76,0.04)_0%,transparent_50%)]">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute -right-20 top-1/2 -translate-y-1/2 w-[400px] h-[400px] rounded-full border border-gold-400/10 animate-float-slow" style={{ animationDuration: '10s', animationDelay: '2s' }}></div>
        {/* Grid lines */}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 pt-32 pb-24 relative z-10">
        <div className="max-w-4xl">
          ...
          {/* Tagline */}
          <p className="text-base lg:text-lg font-body text-muted-foreground max-w-xl leading-relaxed mb-10 reveal opacity-0 translate-y-[30px]" style={{ fontWeight: 300 }}>
          ...
          <div className="flex flex-wrap gap-4 reveal opacity-0 translate-y-[30px]">
            <a href="#services" className="inline-flex items-center gap-3 px-7 py-3.5 text-xs uppercase tracking-widest font-medium text-black transition-all duration-300 hover:shadow-lg hover:shadow-gold-400/20 hover:-translate-y-0.5 bg-gold">
              Our Services
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
            </a>
            <a href="#contact" className="inline-flex items-center gap-3 px-7 py-3.5 text-xs uppercase tracking-widest font-medium transition-all duration-300 border hover:border-gold-400 dark:hover:text-stone-100 hover:text-stone-900 border-gold/30 text-gold">
              Free Consultation
            </a>
          </div>
        </div>
        ...
        {/* Stats row */}
        <div className="mt-20 pt-12 border-t border-border grid grid-cols-2 lg:grid-cols-4 gap-8 reveal opacity-0 translate-y-[30px]">
          <div>
            <div className="font-heading text-6xl font-light text-gold leading-none">4<span className="text-2xl">+</span></div>
            <div className="text-xs uppercase tracking-widest text-muted-foreground mt-1">High Courts</div>
          </div>
          <div>
            <div className="font-heading text-6xl font-light text-gold leading-none">10<span className="text-2xl">+</span></div>
            <div className="text-xs uppercase tracking-widest text-muted-foreground mt-1">Tribunals & Forums</div>
          </div>
          <div>
            <div className="font-heading text-6xl font-light text-gold leading-none">7<span className="text-2xl">+</span></div>
            <div className="text-xs uppercase tracking-widest text-muted-foreground mt-1">Practice Areas</div>
          </div>
          <div>
            <div className="font-heading text-6xl font-light text-gold leading-none">∞</div>
            <div className="text-xs uppercase tracking-widest text-muted-foreground mt-1">Client Trust</div>
          </div>
        </div>
      </div>
    </section>
